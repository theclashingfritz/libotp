...
def interrogate(module):
    ...
    cmd += ' -S %(pandadir)s/include/parser-inc -S %(pandadir)s/include -I %(pandadir)s/include -I%(srcdir)s/base -I%(srcdir)s/components -I%(srcdir)s/nametag -I%(srcdir)s/margins -I%(srcdir)s/settings -I%(srcdir)s/secure'
...
for module in ('base', 'components', 'nametag', 'margins', 'settings', 'secure'):
    interrogate(module)

os.chdir(srcdir)
cmd = os.path.join(pandadir, 'bin', 'interrogate_module') + ' -python-native -oc libotp_module.cxx'
cmd += ' -library libotp -module libotp base.in components.in nametag.in margins.in settings.in secure.in'
run_command(cmd)
